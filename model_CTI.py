...
class GraphFlowModel(nn.Module):
            ...
            for i in range(max_atoms):
                ...
                for j in range(edge_total):
                    ...
                    while not valid:
                        # TODO: add cache. Some atom can not get the right edge type and is stuck in the loop
                        # TODO: add cache. invalid bond set
                        if len(invalid_bond_type_set) < 9 and resample_edge <= 100:
                            latent_edge = prior_edge_dist.sample().view(1,-1)
                            if self.dp:
                                latent_edge = self.flow_core.module.reverse(cur_node_features, cur_adj_features,latent_edge,mode=1, edge_index=torch.Tensor([[j + start, i]]).long().cuda()).view(-1)  # (4, )
                            else:
    ...
    def initialize_masks(self, max_node_unroll=38, max_edge_unroll=10):
        ...
        Args:
            ...
            max edge unroll: maximal number of edges to predict for each generated nodes (default: 12, calculated from
        Returns:
